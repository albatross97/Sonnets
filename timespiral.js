...
class TimeSpiral {
    ...
    _calculateBars() {
        const bars = this._data.map(d => {
            const
                ...
                index = d[this._field.index],
                text = d[this._field.text],
                line = d[this._field.line],
    ...
    _renderBars() {
        ...
        const bars = this._g.selectAll(".bar")
            ...
            .join("rect")
            .attr("class", "bar")
            .attr("fill", d => { return this._mycolor(d[this._field.index])})
            .attr("opacity", 1)
            ...
            .attr("height", d => {
                var hhh = d.size;
                var output = 0;
                if (checklove && d.text.includes("love")) {output += hhh;} 
                if (checkbeauty && d.text.includes("beauty")) {output += hhh;}
                if (checktime && d.text.includes("time")) {output += hhh;}
                if (output) {return output}
                if (checklove === false && checkbeauty === false && checktime === false) {return hhh}
                else {return 0.3*hhh}
            })
            // .attr("height", d => d.size)
            .attr("transform", d => `rotate(${d.angle},${d.x},${d.yr})`)
            .on("mouseover", (e, d) => {
                bars.transition().duration(150).attr("opacity", _ => _ === d ? 1 : 0.3)
                    .attr("height", _ => _ === d ? d.size*1.5 : _.size)
                    .attr("width", _ => _ === d ? w*6 : w);
                renderPoem(d.index, +d.date);
            })
            .on("mouseout", (e, d) => {
